		...
		if (len == 0) {
			...
			if (len == 2) {
				if (API.isIntegerPositive(args[1]) || args[1].equals("-1")) {
					...
					if (index == -1 || ObjectAPI.isListHasIndex(pp.getTitles(), index)) {
						String main;
						if (index == -1) {
							main = "없음";
						} else {
							main = pp.getTitles().get(index);
						}
						pp.setRepresentativeTitle(index);
						...
						if (VaultHook.isChatHook) {
							...
							if (RsPrefix.displayMode == DisplayMode.PREFIX) {
								VaultHook.chat.setPlayerPrefix((Player) sender, main);
							} else if (RsPrefix.displayMode == DisplayMode.SUFFIX) {
				...
				String main = pp.getRepresentativeTitle();
			...
			API.sendMessageList(sender, pp.getTitles(), index, "prefix list");
		}  else {
